...
- `GET /catalog/:id?page=0` — товары категории с пагинацией по 20, фильтрацией и сортировкой (API 1.2.0).
...
## Фильтрация и сортировка каталога (API 1.2.0)

`GET /catalog/:id` принимает дополнительные query-параметры:

| Параметр | Значения | По умолчанию | Описание |
|----------|----------|--------------|----------|
| `in_stock` | `in_stock` / `out_of_stock` / `all` | `all` | Наличие на складе (в наличии / под заказ / все) |
| `price_min` | число ≥ 0 | — | Нижняя граница цены |
| `price_max` | число ≥ 0 | — | Верхняя граница цены |
| `sort_by` | `price_asc` / `price_desc` / `popularity` / `status` | `popularity` | Сортировка |

- Фильтр и сортировка по цене используют **эффективную цену** (`sale_cost`, если действует скидка, иначе `cost`).
- `popularity` — по количеству оформленных заказов товара (по убыванию).
- `status` — товары в наличии идут первыми.
